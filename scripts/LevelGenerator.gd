...
static func _add_platform(parent: Node2D, top_left: Vector2, size: Vector2, moving: bool = false, travel: Vector2 = Vector2.ZERO, speed: float = 60.0) -> void:
        ...
        visual.size = size
        visual.color = Palette.WOOD if moving else Palette.DIRT
        body.add_child(visual)

        var grass := ColorRect.new()
        grass.size = Vector2(size.x, 20)
        grass.color = Palette.ORANGE_DIM if moving else Palette.GRASS
        body.add_child(grass)

        var grass_edge := ColorRect.new()
        grass_edge.size = Vector2(size.x, 6)
        grass_edge.position = Vector2(0, 16)
        grass_edge.color = Palette.ORANGE if moving else Palette.GRASS_DARK
        body.add_child(grass_edge)
